...
function getValorPadraoParticipacao(tipoVeiculo) {
  ...
  if (tipo.includes('moto')) return 5.5;
  if (tipo.includes('suv')) return 5.0;
  return 4.0; // carro padrão
}
...
function gerarCotacaoPDF() {
  ...
  imgFundo.src = 'template-cotacao.png'; // Substitua pelo URL da sua imagem de fundo
...
,
  ...
  { 
    label: `Reboque após pane/colisão ${textoSeguro(reboqueDescricao)}`, 
    value: reboqueValor !== "0.00" ? formatarValorMonetario(reboqueValor) : "NÃO", 
    xLabel: 17, 
    xValue: 170, 
    y: 127 
  },
  ...
  { 
    ...
    value: vidrosValor !== "0.00" ? formatarValorMonetario(vidrosValor) : "NÃO", 
    xLabel: 17, 
    xValue: 170, 
    y: 183 
  },
...
itens.forEach(item => {
  try {
    ...
    // Salvar o PDF
    doc.save(`Cotacao_Seguro_${numeroCotacao}.pdf`);
  };
